# Dashboard Model Build — `lib/models/dashboard_model.dart`

**Status:** ✅ BUILT — production-ready, defensive parsing models
**File created:** `lib/models/dashboard_model.dart` (14 classes, ~640 lines)
**Backend source of truth:** `application/controllers/Dashboard.php` (sindomon-api-tom)
**Endpoints covered:**
- `GET /api/v1/dashboard/nasional` → `DashboardNasional`
- `GET /api/v1/dashboard/drilldown?polda_id=X` → `DashboardDrilldown`

---

## 1. Class Inventory

### `/nasional` endpoint (`data` object)

| Class | JSON section | Fields |
|---|---|---|
| `DashboardNasional` | top-level `data` | `ringkasan`, `peta`, `sitkamtibmas_terkini` |
| `Ringkasan` | `data.ringkasan` | 11 int fields (personil aktif, formasi ideal, riil, selisih, senjata, senjata layak, sarpras, sarpras baik, satwa K9, amunisi butir, H-90 alert) |
| `PetaNode` | `data.peta[]` | `polda_id`, `nama_polda`, `latitude`, `longitude`, `total_personil`, `total_senjata`, `total_sarpras`, `total_k9` |
| `SitkamtibmasTerkini` | `data.sitkamtibmas_terkini[]` | `sitkamtibmas_id`, `polda_id`, `nama_polda?`, `deskripsi_kejadian`, `level_kritis`, `foto_tkp_url?`, `created_at` |

### `/drilldown` endpoint (`data` object)

| Class | JSON section | Fields |
|---|---|---|
| `DashboardDrilldown` | top-level `data` | `polda`, `personil`, `vakansi`, `logistik`, `sitkamtibmas_terkini` |
| `PoldaInfo` | `data.polda` | `id`, `nama_polda`, `latitude`, `longitude` |
| `PersonilStats` | `data.personil` | `total_aktif`, `total_mutasi`, `total_pensiun` |
| `VakansiStats` | `data.vakansi` | `total_formasi_ideal`, `total_jumlah_riil`, `selisih`, `detail_per_jabatan[]` |
| `DetailJabatan` | `data.vakansi.detail_per_jabatan[]` | `jabatan_id`, `nama_jabatan`, `formasi_ideal`, `jumlah_riil`, `is_alert` (bool) |
| `LogistikStats` | `data.logistik` | `senjata`, `sarpras`, `satwa_k9`, `amunisi` |
| `SenjataStats` | `data.logistik.senjata` | `total`, `layak`, `tidak_layak` |
| `SarprasStats` | `data.logistik.sarpras` | `total`, `baik`, `rusak_ringan`, `rusak_berat` |
| `SatwaStats` | `data.logistik.satwa_k9` | `total` |
| `AmunisiStats` | `data.logistik.amunisi` | `total_butir`, `total_batch`, `h90_alert` |

**Key verification:** all **32 payload keys** emitted by `Dashboard.php` were extracted from the PHP source and cross-checked against the Dart model — **0 missing**.

---

## 2. Defensive Parsing Contract

Every `fromJson` factory is **TOTAL** (provably non-throwing — no input can crash it):

| JSON type | Dart strategy | Fallback |
|---|---|---|
| Integer | `_toInt()`: `num` → `.toInt()`; `String` → `int.tryParse`; `bool` → 0/1 | `0` |
| String | `_toStr()`: `value?.toString()` | `''` (nullable fields keep `null`) |
| Boolean | `_toBool()`: `bool` passthrough; `num` → `!= 0`; String `"true"/"1"` | `false` |
| List of objects | `_mapList()`: skips null/non-map elements via `_asMap()` | `const []` |
| Map | `_asMap()`: accepts any `Map` subtype (normalizes to `Map<String, dynamic>`) | `null` → whole-object defaults |

**Guarantees:**
- **Zero `!` force-unwraps** in the file (verified by scan).
- **Zero `as` casts** on raw JSON values — all field access goes through the helpers.
- One malformed element in a list (e.g., `null` or a string where a map is expected) is **skipped**, not fatal — the rest of the payload still parses.
- Missing `data` object (error envelope `{"status":401,"data":{}}`) degrades to a fully-defaulted instance — the UI renders zeros/empty lists instead of crashing.
- `latitude`/`longitude` stay `String` (matching the DECIMAL→string behavior of PHP `json_encode`) with `hasValidCoordinates` + `latLng` getters on `PetaNode`, mirroring the existing `Polda` model (`lib/models/polda_model.dart`) so the map layer can reuse the same marker-filtering pattern.

---

## 3. Complete Production-Ready Code

This is the exact content of `lib/models/dashboard_model.dart`:
...
## 4. Usage Contract (for the page layer, next step)

```dart
// /nasional — pass the `data` object, NOT the envelope:
final decoded = jsonDecode(response.body);
final model = DashboardNasional.fromJson(decoded['data']);

// /drilldown — same contract:
final model = DashboardDrilldown.fromJson(decoded['data']);

// Map markers (replaces raw-map iteration in dashboard.dart):
markers: model.peta
    .where((n) => n.hasValidCoordinates)
    .map((n) => Marker(point: n.latLng, ...))

// Drill-down popup fetch uses the already-available polda_id:
// GET $apiBaseUrl/api/v1/dashboard/drilldown?polda_id=${node.poldaId}
```
...
| Check | Result |
|---|---|
| Force-unwrap `!` scan | 0 matches ✅ |
| Brace / paren / bracket balance (Python count) | 51/51, 233/233, 73/73 — OK ✅ |
| Backend key cross-check (32 PHP payload keys vs Dart `j['...']` access) | 0 missing ✅ |
